...
public class ArrList <E> {
    ...
    public void insert(E newItem, int k) { // 새 항목을 k-1번쨰 항목 다음에 삽입, 준비한 빈 자리에 원소 삽입하기
    	if (size == a.length)   		   // 배열에 빈 공간이 없으면
    		resize(2*a.length);			   // 배열 크기 2배로 확장
    	for (int i = size-1; i >= k; i--)  a[i+1] = a[i];  // 한 칸씩 뒤로 이동 (제일 뒤에꺼 먼저 한 칸씩 뒤로 이동)
    	a[k] = newItem;
    	size++;
    }
    ...
    public E delete(int k) {  // k번째 항목 삭제
		...
		size--;
		if (size > 0 && size == a.length/4) // 배열에 항목들이 1/4만 차지한다면		삭제한 빈 자리 채우기
